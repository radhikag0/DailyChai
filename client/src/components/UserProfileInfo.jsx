...
import { BadgeCheck, PenBox, MapPin } from 'lucide-react' 
  ...
  return (
    <div className='relative py-4 px-6 md:px-8 bg-white'>
        <div className='flex flex-col md:flex-row items-start gap-6'>
            {/* Profile Picture */}
            <div className='relative w-32 h-32 border-4 border-white shadow-lg absolute -top-16 rounded-full'>
                <img src={user.profile_picture} alt={user.full_name} className='absolute rounded-full z-2 object-cover w-full h-full' />
            </div>
            ...
            <div className='w-full pt-16 md:pt-0 md:pl-36'>
                <div className='flex flex-col md:flex-row items-start justify-between'>
                    ...
                    {/* If user is not on others profile means he is opening his own so edit */}
                    {!profileId && 
                        <button onClick={() => setShowEdit(true)} className='flex items-center gap-2 border border-gray-300 hover:bg-gray-50 px-4 py-2 rounded-lg
